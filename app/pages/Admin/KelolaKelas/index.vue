      ...
      <div class="header-left">
        ...
        <div>
          ...
          <p class="page-subtitle">
            Manajemen daftar kelas dan rombongan belajar
          </p>
        </div>
      </div>
            ...
            <tr v-for="(kelas, index) in filteredKelas" :key="index">
              ...
              <td>
                <span class="badge badge-blue"
                  >{{ kelas.jumlah_siswa || 0 }} Siswa</span
                >
              </td>
              <td>
                <span
                  class="status-badge"
                  :class="kelas.aktif ? 'active' : 'inactive'"
                >
                  <span class="dot"></span>
      ...
      <div class="modal-content" style="max-width: 400px">
        ...
        <div class="modal-body text-center" style="text-align: center">
          ...
          <p style="font-size: 1.05rem; color: #475569">
            ...
            >? Siswa yang ada di kelas ini mungkin akan kehilangan referensi
            kelas mereka.
          </p>
        </div>
  ...
  try {
    ...
    if (kelasData) {
      ...
      kelasList.value = kelasData.map((k) => {
        const matchedGuru = teachers.value.find(
          (g) => g.id === k.wali_kelas_id,
        );
        return {
          id: k.id,
          nama: k.nama_kelas,
...
const saveKelas = async () => {
  ...
  const namaKelasFormatted = form.value.nama.toLowerCase().includes("kelas")
    ? form.value.nama
    : `Kelas ${form.value.nama}`;
  ...
  try {
    ...
    showToast(
      "Gagal menyimpan kelas. Pastikan data sekolah ada di database.",
      "error",
    );
  }
};
...
.student-selection-box:focus-within {
  border-color: #10b981;
  box-shadow: 0 0 0 3px rgba(16, 185, 129, 0.1);
}
.search-inside {
...
.student-list-item:hover .btn-add-mini {
  background: #0284c7;
  color: white;
}
.empty-state {
