    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div>
          <h1 class="page-title">
            {{
              selectedClass ? `Jurnal ${selectedClass.name}` : "Kelola Jurnal"
            }}
          </h1>
          <p class="page-subtitle">
            {{
              selectedClass
                ? `Wali Kelas: ${selectedClass.waliKelas}`
                : "Pantau aktivitas pengisian jurnal siswa per kelas"
            }}
          </p>
        </div>
      </div>
          ...
          >
            ...
            <div class="class-card-footer">
              <span class="view-text"
                >Lihat Jurnal Siswa <Icon name="ph:arrow-right-bold"
              /></span>
            </div>
          </div>
          <div
            v-if="filteredClasses.length === 0"
            class="text-center py-8 text-gray"
                ...
                <td>
                  <div class="progress-container">
                    <div
                      ...
                      :style="{
                        width:
                          (siswa.submitted / siswa.totalJurnal) * 100 + '%',
                      }"
                      :class="getProgressColor(siswa.status)"
                    ></div>
                  </div>
                  <span class="progress-text"
                    >{{ siswa.submitted }} dari
                    {{ siswa.totalJurnal }} aktivitas terisi</span
                  >
                </td>
                <td class="text-right">
                  <button class="btn-detail" @click="lihatDetailSiswa(siswa)">
                    Lihat Detail
                  </button>
                </td>
              </tr>
              <tr v-if="filteredStudents.length === 0">
                <td colspan="4" class="text-center py-8 text-gray">
    ...
    <div v-if="showExportModal" class="modal-overlay">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          ...
          <button class="btn-close" @click="showExportModal = false">
            <Icon name="ph:x-bold" />
          </button>
        </div>
        <div class="modal-body text-center">
          <div style="font-size: 3rem; color: #10b981; margin-bottom: 1rem">
            <Icon name="ph:file-xls-duotone" />
          </div>
          <p style="font-size: 1.1rem; color: #475569">
            Apakah Anda ingin mengunduh data jurnal
            {{ selectedClass ? selectedClass.name : "Semua Kelas" }} dalam
            format Excel?
          </p>
        </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="jurnal-items-grid">
            <div
              class="jurnal-item"
              v-for="(item, index) in selectedStudentDetail"
              :key="index"
            >
              <div
                class="jurnal-item-icon"
                :class="
                  item.filled
              ...
              <div class="jurnal-item-content">
                <h4>{{ item.title }}</h4>
                <p v-if="item.filled" class="text-green-600 text-sm font-bold">
                  <Icon name="ph:check-circle-fill" /> Sudah diisi ({{
                    item.time
                  }})
                </p>
                <p v-else class="text-red-500 text-sm">
                  <Icon name="ph:x-circle-fill" /> Belum diisi
                </p>
              </div>
            </div>
          </div>
...
onMounted(async () => {
  ...
  const { data: kebData } = await supabase
    .from("kebiasaan")
  ...
  if (students && students.length > 0) {
    ...
    if (harianIds.length > 0) {
      const { data: ibadahData } = await supabase
        .from("jurnal_ibadah")
        .select("jurnal_harian_id, status")
      ...
      const { data: kebData } = await supabase
        ...
        .select("jurnal_harian_id, kebiasaan_id, status")
        .in("jurnal_harian_id", harianIds);
      if (kebData) {
        kebData.forEach((k) => {
          if (k.status) {
            if (!kebiasaanMap[k.jurnal_harian_id])
              kebiasaanMap[k.jurnal_harian_id] = [];
...
const lihatDetailSiswa = (siswa) => {
  ...
  const bpId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("bangun"),
  )?.id;
  ...
  habits.push({
    ...
    filled: bpFilled,
    time: bpFilled ? "Hari Ini" : "",
  });

  habits.push({
    title: "Beribadah",
    icon: "ph:hands-praying-duotone",
    filled: siswa.hasIbadah,
    time: siswa.hasIbadah ? "Hari Ini" : "",
  });

  const orId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("olahraga"),
  )?.id;
  ...
  habits.push({
    ...
    filled: orFilled,
    time: orFilled ? "Hari Ini" : "",
  });

  const mkId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("makan"),
  )?.id;
  ...
  habits.push({
    ...
    filled: mkFilled,
    time: mkFilled ? "Hari Ini" : "",
  });

  const blId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("belajar"),
  )?.id;
  ...
  habits.push({
    ...
    filled: blFilled,
    time: blFilled ? "Hari Ini" : "",
  });

  const bmId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("masyarakat"),
  )?.id;
  ...
  habits.push({
    ...
    filled: bmFilled,
    time: bmFilled ? "Hari Ini" : "",
  });

  const tdId = masterKebiasaanList.value.find((k) =>
    k.nama_kebiasaan.toLowerCase().includes("tidur"),
  )?.id;
