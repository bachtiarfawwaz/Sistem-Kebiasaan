    ...
    <div class="hero-card islam-theme">
      <div class="hero-content">
        ...
        <div class="hero-text">
          ...
          <p class="hero-subtitle">Pantau kegiatan sholat 5 waktu anak setiap hari. Berilah tanda centang sesuai dengan tanggalnya.</p>
          <div class="alert-info" style="margin-top: 15px; background: rgba(255,255,255,0.2); padding: 12px; border-radius: 8px; font-size: 0.9rem; line-height: 1.4;">
            ...
            <span style="vertical-align: middle;"><strong>Petunjuk:</strong> Untuk mengubah poin/status kebiasaan harian, pastikan Anda mencentang <strong>kelima</strong> waktu sholat (Lengkap). Jika kurang dari 5, maka hari ini belum terhitung sebagai pencapaian.</span>
          </div>
        </div>
      </div>
        ...
        <div class="today-input-section" v-if="!todayData.saved">
          <div class="input-wrapper">
            ...
            <div class="prayer-checkboxes">
              <label v-for="prayer in prayers" :key="prayer.key" class="prayer-checkbox-label" :class="{ 'is-checked': todayInput[prayer.key] }">
                <input type="checkbox" v-model="todayInput[prayer.key]" class="hidden-checkbox" />
                <Icon :name="prayer.icon" class="prayer-icon" />
                <span class="prayer-name">{{ prayer.label }}</span>
                <Icon :name="todayInput[prayer.key] ? 'ph:check-circle-fill' : 'ph:circle'" class="check-status-icon" />
              </label>
            </div>
          </div>
        ...
        <table class="prayer-table">
          <thead>
            <tr>
              ...
              <th v-for="prayer in prayers" :key="prayer.key" class="col-prayer">
            ...
            >
              ...
              <td v-for="prayer in prayers" :key="prayer.key" class="td-prayer">
                <div
                  ...
                  :title="day[prayer.key] ? `${prayer.label} ✓` : `${prayer.label} Belum`"
                >
                  <Icon :name="day[prayer.key] ? 'ph:check-circle-fill' : 'ph:minus'" class="check-icon" />
                </div>
...
const fetchMasterIbadah = async () => {
  const { data } = await supabase.from('ibadah').select('*').eq('agama', 'Islam')
...
const generateDays = async () => {
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    let isSaved = false
    let subuh = false, dzuhur = false, ashar = false, maghrib = false, isya = false
...
const saveToday = async () => {
  ...
  try {
    ...
    for (const p of prayers) {
      ...
      // Cari ID master ibadah berdasarkan nama
      let masterIbadah = ibadahList.value.find(m => m.nama_ibadah.toLowerCase().includes(p.key.replace("'", "")))
...
const totalPrayers = computed(() => days.value.reduce((sum, d) => sum + (d.saved ? countDone(d) : 0), 0))
...
.prayer-th-icon { font-size: 1.4rem; }
...
.table-row.row-today { background: #f0fdf4; border-left: 4px solid #10b981; }
.table-row.row-complete { background: #f0fdf4; }
...
.static-check.checked .check-icon { color: #10b981; }
