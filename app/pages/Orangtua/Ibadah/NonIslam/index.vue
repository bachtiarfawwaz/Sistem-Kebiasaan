    ...
    <div class="hero-card nonislam-theme">
      <div class="hero-content">
        ...
        <div class="hero-text">
          ...
          <p class="hero-subtitle">Pantau kegiatan ibadah anak setiap hari. Berilah tanda centang sesuai dengan tanggalnya.</p>
          <div class="alert-info" style="margin-top: 15px; background: rgba(255,255,255,0.2); padding: 12px; border-radius: 8px; font-size: 0.9rem; line-height: 1.4;">
            ...
            <span style="vertical-align: middle;"><strong>Petunjuk:</strong> Untuk mengubah poin/status kebiasaan harian, pastikan Anda mencentang <strong>ketiga</strong> waktu ibadah (Lengkap). Jika kurang dari 3, maka hari ini belum terhitung sebagai pencapaian.</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="isCurrentMonthView" class="today-action-area">
        <div class="today-input-section" v-if="!todayData.saved">
          <div class="input-wrapper">
            <label class="input-label">Catatan Ibadah Hari Ini ({{ todayFormatted }})</label>
            <div class="worship-checkboxes">
              <label v-for="worship in worships" :key="worship.key" class="worship-checkbox-label" :class="{ 'is-checked': todayInput[worship.key] }">
                <input type="checkbox" v-model="todayInput[worship.key]" class="hidden-checkbox" />
                <Icon :name="worship.icon" class="worship-icon" />
                <span class="worship-name">{{ worship.label }}</span>
                <Icon :name="todayInput[worship.key] ? 'ph:check-circle-fill' : 'ph:circle'" class="check-status-icon" />
              </label>
            </div>
          </div>
        ...
        <div class="today-success-section" v-else>
          <div class="success-badge complete">
            ...
            <div class="success-text">
              <strong>Tersimpan!</strong> Anda telah mengisi catatan ibadah hari ini.
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="history-section">
      <div class="section-header">
        <div class="header-left">
          ...
          <h2 class="section-title">Catatan Ibadah — {{ months[selectedMonth] }} {{ selectedYear }}</h2>
        </div>
        ...
        <table class="worship-table">
          <thead>
            <tr>
              ...
              <th v-for="worship in worships" :key="worship.key" class="col-worship">
          ...
          <tbody>
            <tr
              ...
              :class="{
                ...
                'row-missed': isPastDate(day.date) && day.saved && !isComplete(day)
              }"
            >
              <td class="td-tanggal">
                <div class="date-cell">
                  ...
                  <span v-if="isToday(day.date)" class="today-badge">Hari Ini</span>
                </div>
              </td>
              <td v-for="worship in worships" :key="worship.key" class="td-worship">
                <div
                  v-if="!isFutureDate(day.date) && (day.saved || (isPastDate(day.date) && day.saved))"
                  class="static-check"
                  :class="{ 'checked': day[worship.key] }"
                  :title="day[worship.key] ? `${worship.label} ✓` : `${worship.label} Belum`"
                >
                  <Icon :name="day[worship.key] ? 'ph:check-circle-fill' : 'ph:minus'" class="check-icon" />
                </div>
                <div v-else-if="!isFutureDate(day.date) && isPastDate(day.date) && !day.saved" class="static-check missed">
                  <Icon name="ph:x" class="check-icon" />
                </div>
                <div v-else-if="isToday(day.date) && !day.saved" class="static-check pending">
...
const worships = [
  ...
  { key: 'membacaAlkitab', label: 'Membaca Alkitab', icon: 'ph:book-open-text-duotone' },
...
const fetchMasterIbadah = async () => {
  const { data } = await supabase.from('ibadah').select('*').neq('agama', 'Islam')
...
const generateDays = async () => {
  ...
  // Format tanggal awal dan akhir bulan untuk query
  ...
  // Ambil semua jurnal_harian di bulan ini
  ...
  for (let i = 1; i <= daysInMonth; i++) {
    ...
    // Cari apakah ada jurnal harian di tanggal ini
    const dateString = `${selectedYear.value}-${String(selectedMonth.value + 1).padStart(2, '0')}-${String(i).padStart(2, '0')}`
    const harianToday = (listJurnalHarian || []).find(jh => jh.tanggal === dateString)
    
    let isSaved = false
    let doaPagi = false, membacaAlkitab = false, doaMalam = false

    if (harianToday) {
      // Periksa apakah ada record ibadah
      const ibadahHariIni = listJurnalIbadah.filter(ji => ji.jurnal_harian_id === harianToday.id)
      
      // Jika ada jurnal harian, kita anggap saved=true (mungkin belum semua ibadah dicentang)
      if (ibadahHariIni.length > 0) isSaved = true

      // Petakan status berdasarkan nama ibadah
      ibadahHariIni.forEach(ji => {
        const masterIbadah = ibadahList.value.find(m => m.id === ji.ibadah_id)
        if (masterIbadah) {
  ...
  // Reset today input based on today's data if it exists
...
const saveToday = async () => {
  ...
  try {
    // 1. Cek atau Buat Jurnal Harian
    let jurnalHarianId = null
    const { data: cekJH } = await supabase.from('jurnal_harian').select('id').eq('peserta_didik_id', pesertaDidikId.value).eq('tanggal', dateString).single()
    
    if (cekJH) {
      jurnalHarianId = cekJH.id
    } else {
      const { data: newJH, error: errJH } = await supabase.from('jurnal_harian').insert({
    ...
    // 2. Insert atau Update Jurnal Ibadah
    for (const w of worships) {
      const isChecked = todayInput.value[w.key]
      
      // Cari ID master ibadah berdasarkan label
      let searchKey = ''
      if (w.key === 'doaPagi') searchKey = 'pagi'
      if (w.key === 'membacaAlkitab') searchKey = 'alkitab'
      if (w.key === 'doaMalam') searchKey = 'malam'

      let masterIbadah = ibadahList.value.find(m => m.nama_ibadah.toLowerCase().includes(searchKey))
      
      // Jika master ibadah tidak ada, auto-create ke tabel ibadah (sebagai fallback agar tidak error)
      if (!masterIbadah) {
         const sessionCookie = useCookie('user_session')
         const userAgama = sessionCookie.value?.agama || 'Non-Islam'
         const { data: newIb } = await supabase.from('ibadah').insert({
            nama_ibadah: w.label,
            agama: userAgama
         }).select().single()
         if (newIb) {
      ...
      if (masterIbadah) {
        // Cek apakah sudah ada jurnal ibadah ini
        const { data: cekJI } = await supabase.from('jurnal_ibadah')
          .select('id').eq('jurnal_harian_id', jurnalHarianId).eq('ibadah_id', masterIbadah.id).single()
          
        if (cekJI) {
          await supabase.from('jurnal_ibadah').update({ status: isChecked }).eq('id', cekJI.id)
        } else {
...
const completeDays = computed(() => days.value.filter(d => isComplete(d) && d.saved).length)
const totalWorships = computed(() => days.value.reduce((sum, d) => sum + (d.saved ? countDone(d) : 0), 0))
...
.modern-select:hover { background-color: #e2e8f0; }
.modern-select:focus {
  border-color: #8b5cf6;
...
.col-tanggal { width: 120px; text-align: left !important; padding-left: 1.5rem !important; }
.col-worship  { min-width: 140px; }
...
.worship-th-icon { font-size: 1.4rem; }
...
.table-row.row-today { background: #faf5ff; border-left: 4px solid #8b5cf6; }
.table-row.row-complete { background: #faf5ff; }
.table-row.row-missed { background: #fef9f9; opacity: 0.85; }
.table-row.row-future { opacity: 0.5; background: #f8fafc; }

/* Date Cell */
.td-tanggal { padding: 0.9rem 1rem 0.9rem 1.5rem; }
.date-cell {
  display: flex;
  flex-direction: column;
  gap: 0.1rem;
...
.static-check.checked .check-icon { color: #8b5cf6; }
.static-check.missed .check-icon { color: #ef4444; font-size: 1.5rem; }
.static-check.pending .check-icon { color: #f59e0b; font-size: 1.5rem; }
...
.sum-icon.gold   { color: #f59e0b; }
.sum-icon.purple { color: #8b5cf6; }
.sum-icon.blue   { color: #3b82f6; }
...
@media (max-width: 768px) {
  ...
  .worship-checkboxes { grid-template-columns: 1fr; }
}
</style>
