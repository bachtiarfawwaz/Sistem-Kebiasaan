  ...
  <div class="dashboard-guru">
    ...
    <div class="header-section">
      <div>
        ...
        <p class="page-subtitle">Kelola dan pantau aktivitas siswa di kelas Anda.</p>
      </div>
      <div class="header-right">
        <div class="class-label">
          <span class="class-badge">{{ currentClass ? currentClass.nama_kelas : 'Belum Ditugaskan Kelas' }}</span>
        </div>
        
        <div class="filter-group">
          <!-- Global Segmented Filter untuk Grafik Tren -->
          <select v-model="periodFilter" class="modern-select" @change="fetchStudents">
            <option value="hari_ini">Hari</option>
            <option value="minggu_ini">Minggu</option>
            <option value="bulan_ini">Bulan</option>
          </select>

          <select v-model="selectedTahunAjaran" class="modern-select" @change="fetchStudents">
            <option v-for="thn in listTahunAjaran" :key="thn" :value="thn">{{ thn }}</option>
          </select>
          <select v-model="selectedSemester" class="modern-select" @change="fetchStudents">
    ...
    <div class="table-container">
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="isLoading">
            <td colspan="5" class="text-center py-6 text-slate-500">Memuat data siswa...</td>
          </tr>
          <tr v-else-if="students.length === 0">
            <td colspan="5" class="text-center py-6 text-slate-500">Belum ada siswa di kelas ini.</td>
          </tr>
          <tr v-else v-for="(siswa, index) in students" :key="siswa.id">
            <td class="text-center">{{ index + 1 }}</td>
            <td class="fw-bold name-cell">
              {{ siswa.nama }}
              <button class="mobile-action-btn" title="Lihat Detail Jurnal" @click.stop="navigateToDetail(siswa.id)">
              ...
              >
                 ...
                 <ClientOnly v-else>
                   <Line 
                     ...
                     :style="{ width: '120px', height: '35px', margin: '0 auto' }" 
                   />
                   <span v-else class="text-slate-400 text-sm">Tidak ada data</span>
                 </ClientOnly>
              </div>
            </td>
            <td class="text-center">
              <button class="btn-action" title="Lihat Detail Jurnal" @click="navigateToDetail(siswa.id)">
                <Icon name="ph:eye-duotone" class="btn-icon" /> <span class="btn-text">Detail</span>
              </button>
            </td>
          </tr>
        </tbody>
      ...
      <Teleport to="body">
        <div v-if="hoveredStudent" class="custom-overlay" :style="overlayStyle" @mouseenter="clearHideTimeout" @mouseleave="hideOverlay">
          <div class="popup-content">
            <div class="popup-header">
               <div>
                 <h4 class="popup-title">{{ hoveredStudent.nama }}</h4>
                 <p class="popup-subtitle">Detail Aktivitas ({{ periodFilterLabel }})</p>
               </div>
            </div>
            
            <div class="activities-list mt-3">
               <div v-for="(act, idx) in hoveredStudent.activitiesDetail" :key="idx" class="activity-item">
                  <div class="act-label">
                    <Icon :name="act.statusLevel === 2 ? 'ph:check-circle-fill' : (act.statusLevel === 1 ? 'ph:warning-circle-fill' : 'ph:x-circle-fill')" 
                          :class="act.statusLevel === 2 ? 'text-green-500' : (act.statusLevel === 1 ? 'text-yellow-500' : 'text-red-500')" class="act-icon" />
                    <span>{{ act.name }}</span>
                  </div>
...
const selectedTahunAjaran = ref(isGasal ? `${currentYear}/${currentYear + 1}` : `${currentYear - 1}/${currentYear}`)
...
const getStatusColor = (val, isTotal) => {
  if (isTotal) {
    if (val === 7) return { level: 2, color: '#22c55e', bg: 'rgba(34,197,94,0.1)' } // Green
    if (val >= 5 && val < 7) return { level: 1, color: '#eab308', bg: 'rgba(234,179,8,0.1)' } // Yellow
    ...
    if (val === 1) return { level: 2, color: '#22c55e', bg: 'rgba(34,197,94,0.1)' } // Green
    if (val > 0 && val < 1) return { level: 1, color: '#eab308', bg: 'rgba(234,179,8,0.1)' } // Yellow
...
const fetchStudents = async () => {
  ...
  try {
    const { data: kelasData } = await supabase.from('kelas').select('*').eq('wali_kelas_id', guruId.value).single()
    if (!kelasData) {
    ...
    const isCurrentPeriod = (selectedSemester.value === (isGasal ? 'Gasal' : 'Genap')) && 
                           (selectedTahunAjaran.value === (isGasal ? `${currentYear}/${currentYear + 1}` : `${currentYear - 1}/${currentYear}`))
    
    let studentsData = []
    if (isCurrentPeriod) {
      const { data } = await supabase.from('peserta_didik').select('*').eq('kelas_id', kelasData.id)
      studentsData = data || []
    } else {
      ...
      if (historyJurnal && historyJurnal.length > 0) {
        const uniqueStudentIds = [...new Set(historyJurnal.map(h => h.peserta_didik_id))]
        const { data } = await supabase.from('peserta_didik').select('*').in('id', uniqueStudentIds)
        studentsData = data || []
      }
    }
    ...
    const now = new Date()
    let daysToFetch = periodFilter.value === 'hari_ini' ? 1 : (periodFilter.value === 'minggu_ini' ? 7 : 30)
    ...
    studentsData.forEach(s => {
      ...
      dates.forEach(d => {
        studentLogsMap[s.id][d] = {
           totalCompleted: 0,
           activities: HABIT_NAMES.reduce((acc, curr) => { acc[curr] = 0; return acc }, {})
        }
      })
    })
    ...
    if (harianIds.length > 0) {
      ...
      const { data: ibadahLogs } = await supabase.from('jurnal_ibadah').select('jurnal_harian_id').in('jurnal_harian_id', harianIds).eq('status', true)
      const ibadahCounts = {}
      if (ibadahLogs) {
        ibadahLogs.forEach(log => {
          ibadahCounts[log.jurnal_harian_id] = (ibadahCounts[log.jurnal_harian_id] || 0) + 1
        })
      }

      const { data: kebLogs } = await supabase.from('jurnal_kebiasaan').select('jurnal_harian_id, kebiasaan_id, keterangan').in('jurnal_harian_id', harianIds).eq('status', true)
      ...
      for (const hId of harianIds) {
        ...
        const isIslam = !studentObj?.agama || studentObj.agama.toLowerCase().trim() === 'islam'
        ...
        if (kebLogs && masterKebiasaan) {
          HABIT_NAMES.forEach(habitName => {
            ...
            const m = masterKebiasaan.find(k => k.nama_kebiasaan.toLowerCase().includes(habitName.split(' ')[0].toLowerCase()))
            if (m) {
              const logsForHabit = kebLogs.filter(l => l.jurnal_harian_id === hId && l.kebiasaan_id === m.id)
              if (logsForHabit.length > 0) {
                 ...
                 if (habitName.includes('Bangun Pagi')) {
                 ...
                 } else if (habitName === 'Berolahraga' || habitName === 'Gemar Belajar') {
                    let text = ''
                    try {
                    ...
                    try {
                      const parsed = JSON.parse(l.keterangan)
                      let missing = 0
                      if (!parsed.nasi || parsed.nasi === 'Tidak Konsumsi') missing++
                      if (!parsed.lauk || parsed.lauk === 'Tidak Konsumsi') missing++
                      if (!parsed.sayur || parsed.sayur === 'Tidak Konsumsi') missing++
                      if (!parsed.buah || parsed.buah === 'Tidak Konsumsi') missing++
    ...
    if (hoveredStudent.value) {
      const updatedStudent = students.value.find(s => s.id === hoveredStudent.value.id)
      if (updatedStudent) {
...
const exportExcel = async () => {
  ...
  titleCell.value = 'LAPORAN MONITORING 7 KEBIASAAN ANAK INDONESIA HEBAT (KAIH)';
  ...
  const headerRow = sheet.addRow(['No', 'Nama Lengkap', 'Agama', 'Status Jurnal Terbaru']);
  ...
  students.value.forEach((row, index) => {
    const dataRow = sheet.addRow([
      ...
      row.nama,
      row.agama || '-',
      row.statusMainLevel === 2 ? 'Lengkap (7/7)' : (row.statusMainLevel === 1 ? 'Belum Lengkap (5-6)' : 'Kurang (<5)')
    ]);
  ...
  saveAs(new Blob([buffer]), `Data_Siswa_${currentClass.value.nama_kelas.replace(/\s+/g, '_')}.xlsx`);
}
</script>
...
.fw-bold { font-weight: 700 !important; color: #1e293b !important; }
